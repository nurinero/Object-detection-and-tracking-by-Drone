...
with col3:
    st.write("")
st.markdown("<h1 style='text-align: center; color: Orange;'>Data Real-Time Dashboard</h1>", unsafe_allow_html=True)
...
with col1:
    ...
    Data_Mode= st.selectbox("Which type of object tracking",('Follow Object',' Collect Data'))
with col2:    
    st.write("")
with col3:
    st.header("DataFrame Info :")
    if Data_Mode ==' Collect Data' :
        filename= "/media/nouri/Data/Final_pro/Drone_project/ObjectDetecter/ultralytics/database/CSV_DATA_collect_data_mode.csv"
        df=get_data()
        my_table = st.dataframe(df.tail(10)) 

    elif Data_Mode =='Follow Object' :
        filename = "/media/nouri/Data/Final_pro/Drone_project/ObjectDetecter/ultralytics/database/CSV_DATA_following_mode.csv"
        df=get_data()
...
while True:
    ...
    with placeholder.container():
        ...
        my_chart3 = st.line_chart(data=df,x='Date_time', y=['VRL','Error'])
        my_chart3 = st.line_chart(data=df,x='Date_time', y=['UD','Error_v'])    
    time.sleep(.01)
